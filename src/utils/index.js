...
/*
 ...
 自动补全小数点后几位，可以规定小数点长度，不够补齐，多了四舍五入
  target  要改变的值
  point  小数点后保留几位
 */
export function decimalPoint(target, point = 2) {
  if (!target) return '0.00'
  // 判断目标值是否为正常浮点数
  target = target.toString()
  const tempArr = target.toString().split('.')
  let endGoal = null // 最终目标
  if (tempArr.length == 1) {
    // 没有小数点，自动补齐小数点
    const a = Array(point + 1).join(0)
    endGoal = target.concat('.', a)
  } else if (tempArr.length >= 2) {
    // 正常浮点型数据，查看小数位有几位多了删除，少了用0补全
    const len = tempArr[1].toString().length
    const comp = point - len
    if (comp < 0) {
      // 四舍五入
      const start = tempArr[1].substring(0, point - 1)
      const center = tempArr[1].substring(point - 1, point)
      const end = tempArr[1].substring(point)
      const tempSecond = Math.round(Number(center + '.' + end))
      endGoal = tempArr[0].concat('.', start, tempSecond)
    } else if (comp > 0) {
      // 补0
      const a = Array(comp + 1).join(0)
      endGoal = target.concat(a)
    } else {
      // 拼.完事
      endGoal = tempArr[0].concat('.', tempArr[1])
    }
  }
  // console.log(endGoal, parseFloat(endGoal));
  return endGoal
}
